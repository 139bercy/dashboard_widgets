...
def convert_excel_to_json(path):
    ...
    onglet_properties = ["Carte", "Graph", "Nom_indicateur_GP", "Description_mesure"] # + indicateur
    ...
    for panneau in panneaux:
        ...
        for onglet in onglets:
            ...
            for col in onglet_properties:
                dict_onglet[col] = str(df_panneau[col].iloc[0])
        ...
        dict_panneau["onglets"] = [dict_onglet]
        liste_finale += [dict_panneau]
    return liste_finale

json_final = convert_excel_to_json(path = os.path.join(os.getcwd(), "dashboard_widgets-all-in-vue", "dashboard-configuration", "Web_edito v_1.csv"))
    
with open(os.path.join(os.getcwd(), "dashboard_widgets-all-in-vue", "dashboard-configuration", "dashboard-configuration.json"), 'w', encoding="utf8") as f:
    json.dump(json_final, f, indent=4, ensure_ascii=False, sort_keys=False )
with open(os.path.join(os.getcwd(), "dashboard_widgets-all-in-vue", "public", "dashboard-configuration.json"), 'w', encoding="utf8") as f:
    json.dump(json_final, f, indent=4, ensure_ascii=False, sort_keys=False )
