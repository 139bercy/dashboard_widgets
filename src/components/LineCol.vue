<template>
  ...
  <div class="l_col fr-col-12 fr-grid-row">

        <div class="fr-grid-row fr-col-12 fr-mb-2v" data-box="loc" v-if="localisation">
          <div class="fr-col-9">
          <span class="l_box_title fr-text--sm fr-mb-1w">Localisation: </span>
            ...
            <span class="flex fr-text--sm fr-my-0">{{localisation}}</span>
          </div>
          <div class="fr-col-3">
          <span class="l_box_title fr-text--xs fr-mb-1w" style="text-align: end">Mise à jour : {{date}}</span>
          </div>
        </div>

        <div class="fr-col-12 fr-grid-row fr-mb-5v" data-box="number" v-if="currentValues">
          <div class="indicateur_info fr-grid-row" :class="{'fr-col-6' : names.length > 1}" v-for="(n,i) in names" :key="n">
            <div class="fr-text--sm fr-text--bold fr-mt-0" style="margin-bottom: 0;align-self: center">{{names[i]}}</div>
            <div class="l_box_number_container fr-ml-2w">
        ...
        <div class="scale fr-col-12" v-if="isMap">
